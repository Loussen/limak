...
@foreach($dataStorage as $value)
    @if(!isset($value->relUserProducts->users))
        @continue
    @endif
    <tr class="no-b">
        ...
        <td>
            @if($value->weight)
                <button class="btn btn-outline-primary" id="printHawb" data-id="{{$value->id}}">
...
@foreach($dataWaiting as $value)
    ...
    <tr class="no-b">
        ...
        <td>
            <a href="/admin/invoices/waybill/{{$value->id}}" target="_blank" class="btn-info"> Waybill </a>
        </td>
        <td>
