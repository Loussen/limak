                            ...
                            @foreach($data as $value)
                                <tr class="no-b">
                                    ...
                                    <td>
                                        ...
                                        <div style="font-size: 16px;color: #d43838;font-weight: bold;" class="d-none d-lg-block">{{$value->products->price}} TL</div>
                                    </td>
                                    <td>
                                        <h6>Məhsulun 5%-li qiyməti</h6>
                                        <div style="font-size: 16px;color: #d43838;font-weight: bold;" class="d-none d-lg-block">{{(round($value->products->price + $value->products->price*5/100, 2))}} TL</div>
                                    </td>
                                    <td>
                                        <h6>Karqo</h6>
                                        <div style="font-size: 16px;color: #d43838;font-weight: bold;" class="d-none d-lg-block">{{$value->products->extras->cargo_price}} {{$value->products->extras->cargo_price? "TL":"yoxdur"}}</div>
                                    </td>
                                    <td>
                                        <h6>Ümumi qiymət</h6>
                                        <div style="font-size: 16px;color: #56b759;font-weight: bold;" class="d-none d-lg-block">{{$value->products->relUserProducts->price}} AZN</div>
                                    </td>
                                    <td>
                                        <h6>Mesaj</h6>
    ...
    <script>
        ...
        var bindMoreClick = function() {
            ...
            $(document).on('click','.products_advanced', function () {
                $.ajax({
                    'type': 'GET',
                    url: '{{url("/admin/product-rejection-for-accountant/products")}}/' + $(this).attr('data-id') + '/' + $(this).attr('data-product-id'),
        ...
        $(document).on('click', '.transaction_refuse', function() {
            ...
            swal({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: '{{url("/admin/product-rejection-accept-accountant")}}',
                        data: {id: id},
                        success: function(response) {
                            getRoadData();
                        }
        ...
        getRoadData = function () {
            $.ajax({
                'type': 'GET',
                url: "{{url('/admin/product-rejection-for-accountant')}}?pin="+ $('#pinRoad').val()+ '&tel=' + $('#telRoad').val(),
                success: function($response) {
