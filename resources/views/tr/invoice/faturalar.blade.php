            ...
            <div class="">
                <div class="buttons_list content">
                    <ul class="buttons_list">
                        ...
                        <li><a id="anbar" data-status="2" href="{{url('/invoices/get/index?status=2')}}" class="btn-effect">Tüm ANBAR</a></li>
                        <li><a id="anbar" data-status="2" href="{{url('/invoices/get/anbar')}}" class="btn-effect">ANBAR</a></li>
                        <li><a id="anbar" data-status="4" href="{{url('/invoices/get/faturalar')}}" class="btn-effect active ">Faturalar</a></li>
...
@section('content')
    <div class="content">
                ...
                <ul class="right_buttons">
                    ...
                    <li>
                        ...
                        <li>
                            ...
                            <a href="/invoices/waybillFatura/{{$fatura_id}}"
                               target="_blank" style="color: #FFF"> Waybill </a>
                            </p>
                        </li>
                            @endif
                    <li>
                        @php($link = '')
                        @if($fatura_id>0)
                            @php($link = '?fatura_id='.$fatura_id)
                        @endif
                        <a type="button" target="_blank"  class="btn-effect" href="/invoices/get/faturalarExcel{{$link}}" style="border:1px solid green; background-color: green !important;">
                            Excel
                        </a>
                    ...
                    <li>
                        @php($link = '')
                        @if($fatura_id>0)
                            @php($link = '?id='.$fatura_id)
                        @endif
                        <a type="button" target="_blank"  class="btn-effect" href="/anbar-tr{{$link}}">
                            Fatura
                        </a>

                    </li>
                    <li>
                        <a type="button" target="_blank"  class="btn-effect" href="/daily">
                            Günlük Upload
                        </a>
                    </li>
                    ...
                    <li style="display:{{$display}}">
                        <form action="{{url('/invoices/send-fatura')}}" method="post">
                            <input type="hidden" name="fatura_id" value="{{$fatura_id}}">
                            <input value="Faturanı Gönder" type="submit" class="btn-effect blue_border" >

                        </form>
                    </li>
                    @endif;
                    <li style="display:{{$display}}">
                        <form action="{{url('invoices/removeFatura')}}" method="post" id="selected_inputs_fatura_remove">

                            <input value="Faturadan çıkar" type="submit" class="btn-effect orange_inner" >
        ...
        <div class="custom-container">
            <div class="buttons_list content" style="margin-top: 10px;">
                <ul class="buttons_list">
                    <li role="presentation" class="active"><a href="/invoices/get/faturalar" class="btn-effect {{$fatura_id==0 ? 'active' : ''}}">Tüm faturalar</a></li>
                    <?php
                    $fi = 1;
                    ?>
                    @foreach($faturalar as $fatura)
                        <li role="presentation"><a href="/invoices/get/faturalar?fatura_id={{$fatura->id}}" class="btn-effect {{$fatura_id==$fatura->id ? 'active' : ''}}">
                                <?php
                                    if($fatura->liquid_type==1){
                                        echo  'Sivilar';// ('.$fatura->total_weight.'kq)';
                                    }else{
                                       echo  'Fatura '.$fi; //.'('.$fatura->total_weight.'kq)';
                                        $fi++;
                                    }
                                ?>
                                ...
                                @php($last30days=number_format($value->price / $tryToUsd + $value->shipping_price, 2))
                                    ...
                                    <tr class="tr_{{$value->id}}" style="{{$value->by_bus==1 ? 'background:#e07f7f;color:white;' : ''}}{{$value->liquid_type==1 ? 'background:#7fbce0;color:white;' : ''}}">
                                        ...
                                        <td>{{$value->quantity}}</td>
                                            ...
                                            <td><a href="/invoices/get/faturalar?fatura_id={{$value->fatura_id}}">Git</a></td>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 col-sm-12 col-xs-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="product_type_name" id="product_type_name" required placeholder="@lang('invoice/index.product-type')">
