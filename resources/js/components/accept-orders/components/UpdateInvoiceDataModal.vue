            ...
            <div class="limac-modal-body">
                ...
                <div v-if="productData">
                    <div class="row">
                        ...
                        <div class="form-group col-6">
                            <label>İzləmə nömrəsi</label>
                            <input type="text" class="form-control" v-model="formData.orderTrackingNumber = productData.invoices[0].order_tracking_number" placeholder="Misal: 145678"/>
                        </div>
                        <div class="form-group col-6">
                            <label>Sifarişin tarixi</label>
                            <input type="date" class="form-control" v-model="formData.orderDate = productData.invoices[0].order_date"/>
                        </div>
                        <div class="form-group text-right col-12">
                            <button v-on:click="updateInvoiceData()" class="btn btn-success" v-bind:disabled="checkDisabled()">Dəyiş</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    export default {
        name: "update-invoice-data-modal",
        ...
        methods: {
            ...
            updateInvoiceData() {
                ...
                this.$emit('update-invoice-data', this.formData);
            },
            checkDisabled() {
                var result = false;
