...
-->
                    ...
                    <table  class="table table-striped">
                        ...
                        <tr v-for="order of dataWaiting">
                            ...
                            <td>{{order.post_region}} - {{order.post_index}}</td>
                            <td>
...
-->
                    ...
                    <table  class="table table-striped">
                        ...
                        <tbody>
                        <tr v-for="order of dataConfirmed">
                            <td>{{order.name}} {{order.surname}} <b>{{order.uniqid}}</b></td>
                            <td>{{order.phone}} <b>{{order.email}}</b></td>
                            <td>{{order.city}} {{order.district}} {{order.street}} {{order.village}} {{order.home}}</td>
                            <td>{{order.post_region}} - {{order.post_index}}</td>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.getTransferOrdersWaiting();
            this.getTransferOrdersConfirmed();
        ...
        methods: {
            getTransferOrdersWaiting() {
                axios.get('/cp/transfer/orders-waiting?status=0')
                    .then(data => {
                        this.dataWaiting = data.data.data;
            ...
            getTransferOrdersConfirmed() {
                axios.get('/cp/transfer/orders-waiting?status=1')
                    .then(data => {
                        this.dataConfirmed = data.data.data;
                        this.usdToAzn = data.data.usdToAzn;
                        // this.dataLength= this.data.length;
                    })
                    .catch(err => {
