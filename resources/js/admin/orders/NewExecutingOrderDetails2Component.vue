            ...
            <div class="block customer-list">
                <ul class="list-user">
                    ...
                    <li class="button-list block-button-list">
                        ...
                        <div class="input-group">
                            <button @click="backToPayment()" type="button" class="btn-effect green">Karta gerİ qaytar</button>
                        </div>
                    </li>
                </ul>
            </div>
            ...
            <div class="block table-block">
                <table class="table table-responsive table-striped">
                    ...
                    <tr v-for="product in user.products">
                        ...
                        <td>{{product.id}}</td>
                        <td>
                            <a :href="product.extras.link" target="_blank">{{product.extras.link}}</a>
                    ...
                    <tr v-if="user && user.products && user.products.length > 0" class="last_tr">
                        ...
                        <th>
                            {{ parseFloat(user.products.reduce(function(prev, cur) {
                            return prev + NaNToZero(parseFloat(cur.over_price));
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="col-xs-12">
                            <div class="form-group"><!--
                                {{backToCardProducts}}
                                {{backToBalanceProducts}}-->
                                <label>Karta Qaytarılacaq məbləğ</label> <br />
                                <p>
                                    Balansa qaytar <input type="checkbox" v-model="back_to_balance_check" value="1">
                                </p>
                                <b>5%-siz -> {{back_to_card_amount}} TL</b> <br />
                                <b>5%-li  -> {{parseFloat(back_to_card_amount*1.05).toFixed(2)}} TL</b>
                                <hr />
                                <label>Balans Qaytarılacaq məbləğ</label> <br />
                                <b>5%-siz -> {{back_to_balance_amount}} TL</b> <br />
                                <b>5%-li  -> {{parseFloat(back_to_balance_amount*1.05).toFixed(2)}} TL</b>
<!--
        ...
        methods:{
            ...
            handleSelectedProduct(product){
                if(product.selected){
                    ...
                    if(product.rel_user_products.response_payment=='Payed with balance')
                    {
                        this.backToBalanceProducts.push({
                            id: product.id,
                            transaction: product.rel_user_products.transaction_id,
                            payment: product.over_price,
                        });
                    }else{
                        this.backToCardProducts.push({
                            id: product.id,
                            transaction: product.rel_user_products.transaction_id,
                            payment: product.over_price,
                        });
                    ...
                    let indexBalance = this.backToBalanceProducts.findIndex((pr) =>{
                        return pr.id == product.id;
                    });
                    this.backToBalanceProducts.splice(indexBalance, 1);

                    let indexBackToCard = this.backToCardProducts.findIndex((pr) =>{
                        return pr.id == product.id;
                    });
                    this.backToCardProducts.splice(indexBackToCard, 1);
                }
            },
            sendEFT(){
            ...
            backToPayment(){
                console.log(this.backToCardProducts);
                var s = 0;
                $.each(this.backToCardProducts, function(key, value) {
                    console.log(value.payment);
                    s = parseFloat(s + parseFloat(value.payment));
                });
                this.back_to_card_amount = s;

                var sb = 0;
                $.each(this.backToBalanceProducts, function(key, value) {
                    console.log(value.payment);
                    sb = parseFloat(sb + parseFloat(value.payment));
                });
                this.back_to_balance_amount = sb;

                this.show_back_to_card = true;
            },
            sendBackToCard(){
                ...
                if(!this.back_to_card_amount && !this.back_to_balance_amount){
                    this.back_to_card_errors.push('Məbləğ göstərilməyib')
                }
                if(this.backToCardProducts.length == 0 && this.backToBalanceProducts.length == 0){
                    this.back_to_card_errors.push('Heç bir məhsul seçilməyib')
                }
                if( this.back_to_card_errors.length ==0  ){
                    axios.post('/cp/accounts/backToCard2', {
                        products: this.backToCardProducts,
                        balanceProducts: this.backToBalanceProducts,
                        user: this.user.id,
                        reason: this.back_to_card_reason,
                        amountCard: parseFloat(this.back_to_card_amount*1.05).toFixed(2),
                        amountBalance: parseFloat(this.back_to_balance_amount*1.05).toFixed(2),
                        balanceCheck:this.back_to_balance_check
                    }).then((data) => {
            ...
            getOrderDetails(){
                ...
                axios.get(`/cp/getOrderDetailByUser/${this.$route.params.id}?region_id=${this.$route.params.region}&client_id=${this.$route.params.client_id}`)
                    .then((response) => {
