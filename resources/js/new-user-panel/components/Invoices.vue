                ...
                <div class="block">
                    <div class="block-head">
                        ...
                        <ul class="nav navbar-nav navbar-right">
                            ...
                            <li class="dropdown" v-if="clients!=null">
                                <a href="#" data-toggle="dropdown" aria-haspopup="true" class="drop-link"
                                   aria-expanded="true">{{selected_client_name}} <i class="fa fa-angle-down"></i></a>
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                                    <li>
                                        <a v-on:click="getClientInvoices(0)">{{ExWindow ? ExWindow.translator('panel-errors.corporate_clients'): ''}} </a>
                                    </li>
                                    <li v-for="client in clients">
                                        <a v-on:click="getClientInvoices(client.client_id)">{{idToUniq(client.client_id)}} {{client.name}} {{client.surname}}</a>
                                    </li>
                                </ul>
                            </li>
                            <li class="dropdown">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                                    ...
                                    <li>
                                        <a v-on:click="getTypeInvoices('at_custom')">{{ExWindow ? ExWindow.translator('panel-errors.atCustom'): ''}} <span class="badge">{{allAtCustomInvoicesCount ? allAtCustomInvoicesCount : 0}}</span></a>
                                    </li>
                                    <li>
                                    ...
                                    <li>
                                        <a v-on:click="getTypeInvoices('returns')">{{ExWindow ? ExWindow.translator('panel-errors.returns'): ''}} <span class="badge">{{returnInvoicesCount ? returnInvoicesCount  : 0}}</span></a>
                                    </li>
                                    <li>
                        ...
                        <div class="block-border">
                                ...
                                <table class="table table-bordered table-responsive">
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <p>{{ExWindow ? ExWindow.translator('panel-errors.order2'): ''}} №<br><span>0000{{invoice.id}}</span></p>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <button type="button" class="btn-effect" @click="fullData(invoice.id)">{{ExWindow ? ExWindow.translator('panel-errors.look'): ''}}
                                            </button>
                                            <button v-if="invoice.status_id==1 && (invoice.added_by=='user' || invoice.added_by=='depo' )" type="button" class="btn-effect green" data-toggle="modal"
                                                    data-target=".edit" @click="fullData(invoice.id)">{{ExWindow ? ExWindow.translator('panel-errors.edit'): ''}}
                                            </button>
                                            <button v-if="invoice.status_id==1 && invoice.added_by=='user'" type="button" class="btn-effect red" @click="deleteInvoice(invoice.id)">{{ExWindow ? ExWindow.translator('panel-errors.delete'): ''}}</button>

                                        </td>
                                    </tr>
                                    <tr v-if="selected_invoice_id==invoice.id && selected_invoice">
                                        <td>
                                            <p>{{ExWindow ? ExWindow.translator('panel-errors.trackingNo'): ''}}<br><span>{{selected_invoice.order_tracking_number}}</span></p>
                                        </td>
                                        <td class="product">
                                            <p>{{ExWindow ? ExWindow.translator('panel-errors.invoices_quantity'): ''}}<br><span>{{selected_invoice_count}}</span></p>
                                            <p>{{ExWindow ? ExWindow.translator('panel-errors.weight'): ''}}<br><span>{{selected_invoice.weight}}kq</span></p>
                                        </td>
                                        <td>
                                            <p>{{ExWindow ? ExWindow.translator('panel-errors.delivery_price'): ''}}<br><span>{{selected_invoice.shipping_price}}$ ( {{selected_invoice.shipping_price_azn}} AZN)</span></p>
                                        </td>
                                        <td>
                                            <p>{{ExWindow ? ExWindow.translator('panel-errors.price'): ''}}<br><span>{{selected_invoice.price}}
                                                <span v-if="selected_invoice.country_id==2">USD</span>
                                                <span v-else>TL</span>
                                            </span></p>
                                        </td>
                                        <td class="text-center">
                                            <button type="button" class="btn-effect follow_order"
                                                    data-toggle="modal" data-target=".follow">{{ExWindow ? ExWindow.translator('panel-errors.track_order'): ''}}
                                            </button>
                                        </td>
                                    </tr>
                                    </tbody>
                                        ...
                                        <td>
                                            <div v-if="selected_invoice.packages.length>=1" v-for="package in selected_invoice.packages">
                                                <p v-if="package.products!=null">
                                                    ...
                                                    <a :href="package.products.extras.link" v-if="package.products.extras!=null" target="_blank">
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <table v-if="selected_invoice" class="table table-responsive">
                                                ...
                                                <tr>
                                                    <td>
                                                        <p>{{ExWindow ? ExWindow.translator('panel-errors.order2'): ''}} № <br><span>0000{{selected_invoice.id}}</span></p>
                                                    </td>
                                                    <td>
                                                        <p>{{ExWindow ? ExWindow.translator('panel-errors.date'): ''}} <br><span>{{selected_invoice.order_date}}</span></p>
                                                    </td>
                                                    <td>
                                                        <p>{{ExWindow ? ExWindow.translator('panel-errors.shop'): ''}} <br><span>{{selected_invoice.packages[0].products.shop_name}}</span></p>
                                                    </td>
                                                    <td>
                                                        <p>{{ExWindow ? ExWindow.translator('panel-errors.status'): ''}}<br><span>{{selected_invoice.invoice_status.name}}</span></p>
                                                    </td>
                                                </tr>
                                                </tbody>
                                            </table>
                                        ...
                                        <div class="modal-body follow-order">
                                            <div class="follow-line">
                                                <ul v-if="selected_invoice && selected_invoice.dates!=null">
                                                    ...
                                                    <li :class="returnStatus( selected_invoice.dates,11 ) ? 'active' : ''">
                                                        <span class="warehouse"></span>
                                                        <p><b>{{ExWindow ? ExWindow.translator('panel-errors.atCustom') : '' }}</b>{{ returnStatusDate( selected_invoice.dates,11 ) }}</p>
                                                    </li>
                                                    ...
                                                    <li :class="returnStatus( selected_invoice.dates,16 ) ? 'active' : '' ">
                                                        <span class="shipped"></span>
                                                        <p><b>{{ExWindow ? ExWindow.translator('panel-errors.returns') : '' }}</b>{{ returnStatusDate( selected_invoice.dates,16 ) }}</p>
                                                    </li>
                            ...
                            <div class="modal fade edit" id="myModal1" role="dialog"
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h4>{{ExWindow ? ExWindow.translator('panel-errors.edit') : '' }}</h4>
                                            <button type="button" class="close" data-dismiss="modal"
                                            ...
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <div class="btn-part text-left">
                                                        ...
                                                        <button @click="send" class="btn-effect" data-dismiss="modal">
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.getCounts();
            this.getCurrency();
            this.getTypeInvoices("all");
            this.getClients();
            //this.getAllInvoicesCount();

            this.ExWindow = window;
            this.day = (new Date()).getDate();
            this.selected_client_name = this.ExWindow && this.ExWindow.translator('panel-errors.corporate_clients');
        ...
        methods: {
            ...
            send(){
                ...
                if(Object.values(this.errors).every((el) => el == null)){
                    ...
                    data.append('product_id', this.selected_product_id);
                    ...
                    axios({url: '/site/invoice/edit', method: 'POST', data: data}).then(response => {
                        swal({
                            ...
                            //yuklendi
                            this.fullData(this.selected_invoice_id);
                        })
                    });
                }
            },
                ...
                if(this.selected_invoice_id==i_id){
                    ...
                    axios.get(path)
                        .then(data => {
                            ...
                            this.selected_invoice_count = data.data.data.packages[0].products.quantity;

                            if(data.data.data.shipping_price==null){
                                this.selected_invoice.shipping_price_azn = null;
                            }else{
                                this.selected_invoice.shipping_price_azn = parseFloat(this.selected_invoice.shipping_price*this.currency).toFixed(2);
                            }

                            this.shop = data.data.data.packages[0].products.shop_name;
                            this.selected_product_id = data.data.data.packages[0].products.id;
            ...
            async getClientInvoices(client_id)
            {
                this.selected_client_id = client_id;
                if(client_id>0){
                    this.selected_client_name = this.idToUniq(this.clients[client_id].client_id)+' '+this.clients[client_id].name;
                }else{
                    this.selected_client_name = this.ExWindow && this.ExWindow.translator('panel-errors.corporate_clients');
                }
                this.getTypeInvoices(this.selected_type);
            ...
            async getTypeInvoices(type) {
                ...
                if(type == 'waiting'){
                    ...
                    this.balance = invoices.data.balance;
                ...
                }else if(type == 'at_custom'){
                    const invoices = await this.getInvoices('at_custom');

                    this.allInvoices = invoices.data.data;

                    this.selected_type_name = this.ExWindow && this.ExWindow.translator('panel-errors.atCustom');

                    this.selected_type_count = this.allAtCustomInvoicesCount;
                    this.balance = invoices.data.balance;
                    ...
                    const invoices = await this.getInvoices('returns');

                    this.allInvoices = invoices.data.data;

                    this.selected_type_name = this.ExWindow && this.ExWindow.translator('panel-errors.returns');

                    this.selected_type_count = this.returnInvoicesCount;
                    this.balance = invoices.data.balance;
            ...
            getInvoices(type) {
                ...
                var path = '/panel/get-invoices/' + type + '?country_id=' + this.country.id+'&client_id='+this.selected_client_id;
                return axios.get(path);
