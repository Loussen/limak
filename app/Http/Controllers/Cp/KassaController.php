...
class KassaController extends BaseController
{
    ...
    public function add(Request $request)
    {
        ...
        if(number_format(($balance_payment + $terminal_payment + $cash_amount),2) >= $total_price){
            ...
            if($terminal_payment > 0){
                $this->log(23,$terminal_payment,'plus',0, 0,$user_id,$invoices_string, 'Terminal Ödəniş');
            }
            if($courier_price > 0){
            ...
            if($user->balance < 0){
                LogBalance::create([
                    ...
                    'new_balance' => 0,
                    'money' => ($user->balance - 2*$user->balance),
                    'message' => "Kassadan ödəniş",
                    'type' => 'azn'
                ]);
    ...
    public function show(Request $request){
        ...
        $cash = Account::find($account_id);
        return response()->json(['status' => 200, 'balance' => $cash->balance, 'balance_try' => $cash->balance_try]);
    }
    ...
    public function store(Request $request){
        ...
        $message .= ($log_type == 0 )? ' Səhv əməliyyat':'';
        $message .= " ".$reason;
    ...
    public function storeTry(Request $request){
        ...
        $account_id = $request->account_id;
        $cash = Account::find($account_id);
        $before_payment = $cash->balance_try;
        $cash_amount = ($type == 'minus') ? $amount - 2*$amount : $amount;
        $message = ($type == 'minus') ? 'TRY Kassadan məxaric' : 'TRY Kassaya mədaxil' ;
        $cash->balance_try += $cash_amount;
        if($cash->save()) $this->log($cash->id,$amount,$type,$before_payment,$cash->balance_try,0,0, $message);
    ...
    public function updateUserBalanceTry(Request $request)
    {
        ...
        $cash_amount = ($type == 'minus') ? $amount - 2*$amount : $amount;
        $cash_account = Account::find($account_id);
        $before_payment = $cash_account->balance_try;
        $cash_account->balance_try = $cash_account->balance_try + $cash_amount;
        if($cash_account->save()){
            $this->log($cash_account->id,$cash_amount,$type,$before_payment,$cash_account->balance_try,$user_id,'', $message);
        }
...
//            'data' =>
    ...
    public function kassaBack(Request $request){
        $amount=$request->amount;
        $reason=$request->reason;
        $user_id=$request->uniqid;
        $account_id = $request->account_id;
